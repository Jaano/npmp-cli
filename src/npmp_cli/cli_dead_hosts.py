...
dead_host_app = typer.Typer(no_args_is_help=True)


@dead_host_app.command("list")
def dead_host_list(json_out: bool = typer.Option(False, "--json", help="Print JSON")) -> None:
    with client_context(readonly=True) as client:
        items = list(client.list_dead_hosts().values())
        ...
        for x in items:
            ...
            print(f"{x.id}\t{int(bool(x.enabled))}\t{domains}")


@dead_host_app.command("show")
def dead_host_show(
    identifier: str = typer.Argument(..., help="Host id or domain"),
    json_out: bool = typer.Option(False, "--json", help="Print JSON"),
) -> None:
    with client_context(readonly=True) as client:
        item = resolve_dead_host(client, identifier)
        if json_out:
...
@dead_host_app.command("create")
def dead_host_create(
    ...
    from .models import DeadHostItem
    ...
    with client_context(readonly=dry_run) as client:
        ...
        if client.get_dead_host_id(natural) > 0:
            raise typer.BadParameter("dead-host already exists for domain(s); use update")
        item = DeadHostItem(
            api=client,
        ...
        if dry_run:
            print_json({"action": "create", "kind": "dead-host", "payload": payload})
            return
        res = client.create_dead_host(payload)
        print_json(res)


@dead_host_app.command("update")
def dead_host_update(
    identifier: str = typer.Argument(..., help="Host id or domain"),
    ...
    with client_context(readonly=dry_run) as client:
        existing = resolve_dead_host(client, identifier)
        host_id = int(existing.id)
        item = DeadHostItem.from_json(client, existing.to_json())
        ...
        payload = item.to_payload()
        if dry_run:
            print_json({"action": "update", "kind": "dead-host", "id": host_id, "payload": payload, "take_ownership": take_ownership})
            return
        ...
        res = client.update_dead_host(host_id, payload)
        print_json(res)


@dead_host_app.command("delete")
def dead_host_delete(
    identifier: str = typer.Argument(..., help="Host id or domain"),
    ...
    with client_context(readonly=dry_run) as client:
        item = resolve_dead_host(client, identifier)
        host_id = int(item.id)
        require_force_if_interactive(force=force, prompt=f"Delete dead-host {host_id} ({','.join(item.domain_names)})?")
        if dry_run:
            print_json({"action": "delete", "kind": "dead-host", "id": host_id})
            return
        item.delete()


@dead_host_app.command("enable")
def dead_host_enable(identifier: str = typer.Argument(..., help="Host id or domain")) -> None:
    with client_context() as client:
        item = resolve_dead_host(client, identifier)
        client.enable_dead_host(int(item.id))


@dead_host_app.command("disable")
def dead_host_disable(identifier: str = typer.Argument(..., help="Host id or domain")) -> None:
    with client_context() as client:
        item = resolve_dead_host(client, identifier)
        client.disable_dead_host(int(item.id))
