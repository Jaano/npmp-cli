...
@dataclass
class DeadHostItem:
    kind: ClassVar[Kind] = Kind.DEAD_HOSTS
    ...
    @classmethod
    def from_json(cls, api: NPMplusClient, payload: Mapping[str, Any]) -> DeadHostItem:
    ...
    def save(self, *, take_ownership: bool = False) -> tuple[Literal["create", "update"], dict[str, Any]]:
        found_id = self.api.get_dead_host_id(self.natural_index)
        effective_id = found_id if found_id > 0 else None

        replace = False
        if take_ownership and effective_id is not None:
            existing = self.api.list_dead_hosts().get(effective_id)
        ...
        def _apply_enabled(result: Mapping[str, Any]) -> None:
            ...
            if current_enabled == self.enabled:
                return
            if self.enabled:
                self.api.enable_dead_host(host_id)
            else:
                self.api.disable_dead_host(host_id)

        if effective_id is None:
            result = self.api.create_dead_host(self.to_payload())
            self.id = utils.normalize_int(result.get("id"))
            _apply_enabled(result)
            return "create", result
        if replace:
            self.api.delete_dead_host(effective_id)
            result = self.api.create_dead_host(self.to_payload())
            self.id = utils.normalize_int(result.get("id"))
            _apply_enabled(result)
            return "create", result
        ...
        try:
            result = self.api.update_dead_host(effective_id, self.to_payload())
            self.id = utils.normalize_int(result.get("id"))
            _apply_enabled(result)
    ...
    def delete(self) -> None:
        ...
        self.api.delete_dead_host(self.id)
