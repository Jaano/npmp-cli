...
from .specs import (
    ...
    DockerDeadHostSpec,
    domain_key,
    find_orphan_dead_hosts,
    index_dead_hosts_by_domains,
...
def sync_docker_dead_hosts(
    *,
    ...
    docker_specs: Sequence[DockerDeadHostSpec],
    ...
    for docker_spec in docker_specs:
        ...
        if key in seen_domain_keys:
            ...
            logger.warning(
                "Duplicate domain_names in docker specs; skipping dead-hosts domains=%s",
                natural_index,
            )
            continue
        ...
        desired = DeadHostFields.item_from_docker_spec(docker_spec, client=client)
        ...
        if existing_item is not None and DeadHostFields.items_equal(desired, existing_item):
            logger.info(
    ...
    if disable_orphans and effective_owner_id is not None:
        if not seen_domain_keys:
            logger.warning("disable_orphans requested but no dead-host specs provided; skipping orphan disable")
        else:
    ...
    if delete_orphans and effective_owner_id is not None:
        if not seen_domain_keys:
            logger.warning("delete_orphans requested but no dead-host specs provided; skipping orphan deletion")
        else:
            orphans = find_orphan_dead_hosts(
                existing_items=existing,
                managed_domain_keys=seen_domain_keys,
