...
from npmp_cli.docker_scanner import extract_proxy_host_spec_from_labels, extract_proxy_host_specs_from_inspect
...
def test_extract_proxy_host_spec_from_labels_requires_all_fields() -> None:
	...
	assert (
		extract_proxy_host_spec_from_labels(
			labels=labels,
			container_id="abc",
			container_name="/c",
		)
...
def test_extract_proxy_host_spec_from_labels_parses_values() -> None:
	...
	spec = extract_proxy_host_spec_from_labels(labels=labels, container_id="abc", container_name="/my")
...
def test_extract_proxy_host_specs_from_inspect() -> None:
	...
	specs = extract_proxy_host_specs_from_inspect(inspect)
	assert len(specs) == 1
